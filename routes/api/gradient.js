...
router.get("/gradients", async (req,res)=>{
    try {
...
router.post("/gradients", async (req,res)=>{
    try {
...
router.put("/gradients/:id",async (req,res)=>{
    try {
        const gradient = await Gradient.findByIdAndUpdate(req.params.id,req.body,{new:true});
        res.send(gradient);
    } catch (error) {
        console.log(error.message);
        return res.status(500);  
...
router.delete("/gradients/:id", async (req,res)=>{
    try {
...
router.get("/gradients/downloads/:themename", async (req,res)=>{
